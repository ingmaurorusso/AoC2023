...
auto day19Part1(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    for (const auto& part : parts) {
        ...
        while (!(accepted || rejected)) {
            if (workflows.size() <= ++cycling) {
                throw std::runtime_error(
                    "infinite cycling (likely bad input) for part n. "
                    + std::to_string(partIndex));
            }

            bool okSomeRule = false;
            for (const auto& rule : itW->second) {
                ...
                if (feat == Feature::NoRule) {
                    ...
                    auto value = part.at(toUnderlying(feat));
                    if (value == thres) {
                        // continue to next rule, cannot be smaller nor higher.
                        continue;
                    }

                    if ((value > thres) == higher) {
                        matchedRule = true;
                    }
                }
                ...
                if (matchedRule) {
                    ...
                    if (rule.dest == "A") {
                        ...
                        const auto itWnew = workflows.find(rule.dest);
                        if (itWnew == workflows.end()) {
                            throw std::invalid_argument(
                                "Absent workflow named " + rule.dest + " invoked from "
                                + itW->first);
                        }
                        itW = itWnew;
                    }
            ...
            if (!okSomeRule) {
                throw std::runtime_error(
                    "no rule matched for part n. " + std::to_string(partIndex) + " in workflow "
                    + itW->first);
            }
        }
        ...
        if (accepted) {
            ...
            throw std::runtime_error(
                "no acceptance, no rejection for part n. " + std::to_string(partIndex));
        }
    }
...
int main()
{
    try {
        ...
        // day19Part1("./19_input_file.txt",true);
