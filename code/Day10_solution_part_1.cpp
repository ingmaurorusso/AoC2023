...
//TODO: change into a struct with 'p1' and 'p2' (more readable code).
using Near = std::pair<Point,Point>;

} // namespace

auto day10Part1(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    while (inputStream->getline(cc.data(), MaxLineLength, '\n')) {
        ...
        for (auto ch : line) {
            ...
            case '.':
                break;
            case '-':
                network[p] = std::make_pair(Point{x - 1U, y}, Point{x + 1U, y});
                break;
            case '|':
                network[p] = std::make_pair(Point{x, y - 1U}, Point{x, y + 1U});
                break;
            case 'L':
                network[p] = std::make_pair(Point{x, y - 1U}, Point{x + 1U, y});
                break;
            case 'F':
                network[p] = std::make_pair(Point{x, y + 1U}, Point{x + 1U, y});
                break;
    ...
    for (const auto& [p, near] : network) {
        if ((near.first == start) || (near.second == start)) {
            switch (++nNearToStart) {
            case 1U:
                p1 = p;
                break;
    ...
    if (nNearToStart < 2U) {
        ...
        throw std::invalid_argument("too few near to start: " + std::to_string(nNearToStart));
    } // else:
    network[start] = std::make_pair(p1, p2);
    ...
    while ((scroll != start) || (loopMax == 0U)) {
        ...
        if (goodPoint(near.first)) {
            prev = scroll;
            scroll = near.first;
        } else if (goodPoint(near.second)) {
            prev = scroll;
            scroll = near.second;
        } else {
            throw std::invalid_argument(
                "no move possible anymore at the step n. " + std::to_string(loopMax));
        }

        // std::cout << "Moved to " << pointToStr(scroll) << std::endl;
    }

    std::cout << "Number of lines: " << lineCount << std::endl;
    std::cout << "Start point " << pointToStr(start) << "\n";
    std::cout << "Max loop " << loopMax << std::endl;

    auto res = ((loopMax + 1U) / 2);
    std::cout << "Result Day 10 p.1 : " << res << "\n\n\n";
    return res;
}
...
int main10p1()
{
    try {
        ...
        // day10Part1("./10_input_file.txt",true);
