...
auto day22Part2(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    /*static const auto hashBrick = [](const Brick & brick){
        return brick.at(0).x * 100000UL + brick.at(0).y * 100000L + brick.at(0).z * 1000UL +
                brick.at(1).x * 100UL + brick.at(1).y * 10L + brick.at(1).z;
    };
    using Bricks = std::unordered_set<Brick,decltype(hashBrick)>;*/
    //NOLINTNEXTLINE(readability-magic-numbers)
    static const auto hashPoint = [](const Point& p) { return p.x * 1000UL + p.y; };

    using BrickIdx = unsigned long;
    using BricksMap = std::map<Brick, BrickIdx>;
    BricksMap bricks; // need ordered, by increasing z.

    std::unordered_map<Point, std::map<Coord, BrickIdx>, decltype(hashPoint)> zsPerXY(
        0U, hashPoint);

    using BricksIdx = std::unordered_set<BrickIdx>;

    BricksIdx bricksBelowOthersAsSingle;

    std::unordered_map<BrickIdx, BricksIdx> bricksSustaining;
    std::unordered_map<BrickIdx, BricksIdx> bricksSustained;
    ...
    while (inputStream->getline(cc.data(), MaxLineLength, '\n')) {
        ...
        while (true) {
            zsPerXY[scrollBrickXY.first].emplace(brick.at(1).z, 0U);
    ...
    BrickIdx brickIdx = 0U;

    // bricks are naturally sorted by incrasing 'z', based on the Point3 compare operator.
    for (auto itB = bricks.begin(); itB != bricks.end();) {
        itB->second = ++brickIdx; // not a problem starting with one.

        const auto& brick = itB->first;
        Coord z = brick.at(0).z; // '0' is the lowest in any case

        Coord maxZ = 0U;

        BricksIdx bricksBelow;

        if (z > 1U) { // speed-up
            auto scrollBrickXY = brickFirstXY(brick);
            const auto lastScrollBrick = brickLastXY(brick);
            while (true) {
                auto& vertZsSet = zsPerXY[scrollBrickXY.first];

                auto itZ = vertZsSet.lower_bound(z);
                if (itZ != vertZsSet.begin()) {
                    --itZ;
                    // There is a base, update maxZ but without ending the cycle.

                    const auto brickZ = itZ->first;
                    const auto& brickIdxBelow = itZ->second;
                    ...
                    if (isBrickBelow) {
                        bricksBelow.insert(brickIdxBelow);
                    }
                }
            ...
            if (bricksBelow.size() == 1U) {
                const auto brickIdxBelow = *bricksBelow.cbegin();
                bricksBelowOthersAsSingle.insert(brickIdxBelow);
                // std::cout << brickToStr(brickBelow) << " single under " <<
                // brickToStr(brick) << '\n';
                // std::cout << brickIdxBelow << " single under " << brickIdx << '\n';
            }

            for (const auto brickIdxBelow : bricksBelow) {
                bricksSustained[brickIdxBelow].insert(brickIdx);
            }

            bricksSustaining[brickIdx] = std::move(bricksBelow);
        }

        // bricksBelow is the set of the ones below (after moving if maxZ+1U < z).
        const auto itBnext = std::next(itB);
        if (maxZ + 1U < z) { // only for turn == 1.
            ...
            Coord oldHighZ = newBrick.at(1).z;
            newBrick.at(1).z -= (z - newZ);
            if (bricks.count(newBrick) > 0) {
                throw std::runtime_error("repeated newBrick");
            }
            bricks.emplace(newBrick, brickIdx);
            // not really needed to record brickIdx. It may also be lower than the
            // index associated to bricks already scrolled, but it does not matter;
            // what matters is the unicity.

            // even the data in bricksPerXY has to be modified.
            auto scrollBrickXY = brickFirstXY(newBrick);
            const auto lastScrollBrick = brickLastXY(newBrick);
            while (true) {
                auto& vertZsSet = zsPerXY[scrollBrickXY.first];

                vertZsSet.erase(oldHighZ);
                vertZsSet.emplace(newBrick.at(1).z, brickIdx);
            ...
            bricks.erase(itB);
            // the removed position is higher than the new one which, therefore,
            // will not be scrolled.
        } else {
            // need to fix in zsPerXY as this map initially included all zeroes.
            auto scrollBrickXY = brickFirstXY(brick);
            const auto lastScrollBrick = brickLastXY(brick);
            while (true) {
                auto& vertZsSet = zsPerXY[scrollBrickXY.first];

                vertZsSet[brick.at(1).z] = brickIdx;
    ...
    std::cout << "Bricks below as single of others : n. " << bricksBelowOthersAsSingle.size()
                << std::endl;
    Count res = bricks.size() - bricksBelowOthersAsSingle.size();
    std::cout << "Num safe bricks " << res << std::endl;

    // now look for chain effect.
    res = 0U;
    for (brickIdx = 0U; brickIdx < bricks.size(); ++brickIdx) {
        // Try to remove and check how many fall down in a chain.
        auto residualBricksSustaining = bricksSustaining;

        BricksIdx toRemove;
        toRemove.insert(brickIdx);
        Count removed = 0U; // brick is not counted.

        while (!toRemove.empty()) {
            BricksIdx newToRemove;

            for (const auto& brickIdxToRemove : toRemove) {
                for (const auto& brickIdxSustained : bricksSustained[brickIdxToRemove]) {
                    BricksIdx& sustainingToReduce = residualBricksSustaining[brickIdxSustained];
                    sustainingToReduce.erase(brickIdxToRemove);
                    if (sustainingToReduce.empty()) {
                        newToRemove.insert(brickIdxSustained);
                    }
                }
            }

            removed += newToRemove.size(); // cannot remove twice, no circles in z direction.
            toRemove = std::move(newToRemove);
        }
...
int main22p2()
{
    try {
        ...
        // day22Part2("./22_input_file.txt",true);
