...
static const std::map<std::string,unsigned> s_Colors{ // NOLINT
    std::make_pair("red",12U),std::make_pair("green",13U),
    std::make_pair("blue",14U)
    };
static const std::vector<unsigned long> s_zeroMin{0U,0U,0U}; // NOLINT
...
unsigned long day02Part2(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    while (inputStream->getline(cc.data(), MaxLineLength, '\n')) {
        ...
        std::string tokenGame, tokenTitle;
        unsigned gameIndex{};
        ...
        while (true) {
            ...
            case ',':
                ...
                tokenColor.resize(tokenColor.length() - 1);
                break;
            default:
                noComma = true;
            };
            ...
            for (const auto& [color, maxValue] : s_Colors) {
                if (color == tokenColor) {
                    if (extractedColors.count(indexColor) > 0) {
                        throw std::invalid_argument(
                            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                    ...
                    ++nTotalColors;
                    if (number > minCubes[indexColor]) {
                        minCubes[indexColor] = number;
                    }
                    break;
                }
                ++indexColor;
        ...
        size_t power
            = std::accumulate(minCubes.begin(), minCubes.end(), 1U, std::multiplies<>());
    ...
    std::cout << "N. of zero-power games: " << zeroPowerGames << std::endl;
...
{
    try {
        day02Part2(Input, false);
        // day02Part2("./02_input_file.txt",true);
...
Example:
Number of lines: 5
N. of zero-power games: 0
N. of total colors: 35
N. of total extractions: 14
...
Number of lines: 100
N. of zero-power games: 0
N. of total colors: 1179
N. of total extractions: 455
Result: 63542
*/
