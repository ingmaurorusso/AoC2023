...
auto day03Part2()
{
    ...
    unsigned nTotalValues{0U};
    unsigned nTotalTouch{0U};
    unsigned nTotalGoodStars{0U};
    unsigned nTotalBadStars{0U};

    using StartRange = size_t;
    using EndRange = size_t;
    using Value = unsigned long;

    using RangeNumber = std::map<StartRange, std::pair<EndRange, Value>>;
    RangeNumber prevNumberRanges;

    using Stars
        = std::map<size_t, std::set<Value>>; // stars position and set of touched numbers.
    Stars prevStars;

    auto checkStars = [&sum, &nTotalGoodStars, &nTotalBadStars](const Stars& stars) {
        for (const auto& [starPos, touchSet] : stars) {
            if (touchSet.size() == 2) {
                sum += (*touchSet.begin()) * (*touchSet.rbegin());
                ++nTotalGoodStars;
            } else {
                ++nTotalBadStars;
            }
    ...
    while (inputStream.getline(cc.data(), MaxLineLength, '\n')) {
        ...
        for (size_t p = 1U; p <= line.length(); ++p) {
            ...
            if (std::isdigit(ch)) {
                ...
                bool justEndedValue = false;
                if (r > 0U) {
                    // A value just ended followed by '.' or '\n'.
                    justEndedValue = true;
                    ...
                    // keep 'r' and 'v' for the moment.
                }

                if (justEndedValue) {
                    if (const auto it = stars.find(r - 1U); it != stars.end()) {
                        // A start was before the value in the same line.
                        it->second.insert(v);
                        ++nTotalTouch;
                    }

                    for (auto it = prevStars.lower_bound(r - 1U); it != prevStars.end(); ++it) {
                        if (it->first <= p) {
                            // a star was in previous line above the number,
                            // including positions r-1 and p corresponding to diagonal touch.
                            it->second.insert(v);
                            ++nTotalTouch;
                        } else {
                            break;
                ...
                if (ch == '*') {
                    ...
                    if (justEndedValue) {
                        // A value just ended before the star.
                        values.insert(v);
                        ++nTotalTouch;
                    }
                    ...
                    if (!prevNumberRanges.empty()) {
                        auto itNum = prevNumberRanges.lower_bound(p + 1U);
                        // itNum points to the first that starts after p.
                        if (itNum != prevNumberRanges.end()) {
                            if (itNum->first == p + 1) {
                                // a number in previous line starts exactly at 'p+1' (diagonal
                                // touch).
                                values.insert(itNum->second.first);
                                ++nTotalTouch;
                            }
                        }

                        if (itNum != prevNumberRanges.begin()) {
                            --itNum; // begins at most on 'p+1' (or lefter).
                            if (itNum->second.second >= p - 1) {
        ...
        // Ended line: look for 'complete' stars in previous line.
        checkStars(prevStars);

        prevNumberRanges = numberRanges;
        prevStars = stars;
    }

    // Ended line: look for 'complete' stars in last line.
    checkStars(prevStars);

    std::cout << "Number of lines " << lineCount << std::endl;
    std::cout << "N. of total star touch: " << nTotalTouch << std::endl;
    std::cout << "N. of total good stars: " << nTotalGoodStars << std::endl;
    std::cout << "N. of total bad stars: " << nTotalBadStars << std::endl;
...
Example:
Number of lines: 10
N. of total star touch: 5
N. of total good stars: 2
N. of total bad stars: 1
N. of leading zero: 0
N. of zero values: 0
N. of total values: 10
Result: 467835

Real input:
Number of lines: 140
N. of total star touch: 663
N. of total good stars: 309
N. of total bad stars: 45
N. of leading zero: 0
N. of zero values: 0
N. of total values: 1215
Result: 76314915
*/
