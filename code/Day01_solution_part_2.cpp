...
// Exploit the fact that the assignemnt excludes 'zero' as possible string.
constexpr std::array<std::string_view,10> StrNumbers =
...
unsigned long day01Part2(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    auto charsOrDigit = [](const auto inputItFirst, const auto inputItEnd, Direction dir) {
        std::vector<size_t> partiallyFound(
            std::size(StrNumbers), 0U); // how many for each string.

        std::remove_cv_t<decltype(inputItFirst)> scroll = inputItFirst;

        while (scroll != inputItEnd) {
            const auto next = *scroll;
            if (std::isdigit(next)) {
                return std::make_pair(static_cast<size_t>(next - '0'), scroll);
            }

            for (size_t i = 0U; i < partiallyFound.size(); ++i) {
                const auto& s = StrNumbers.at(i);
                if (!s.empty()) {
                    auto& count = partiallyFound[i];
                    auto charIndex
                        = (dir == Direction::Front) ? count : (s.length() - count - 1);
                    if (s[charIndex] == next) {
                        if (s.length() == ++count) {
                            return std::make_pair(i, scroll); // 'one' is in position [1], etc.
                        }
                    } else if (count > 0U) {
                        auto firstCharIndex = (dir == Direction::Front) ? 0 : (s.length() - 1);

                        count = !!(s[firstCharIndex] == next);
                        // Need to check back for the case of 'nine', in case 'nini' is found.
                        // 'nine' is the only string that contains agin the initial character.
                        // A more general code might be written to look for the maximum initial
                        // sub-string of 's' that ends with 'next'.
                        if ((i == iNine) && (count == 0U)) {
                            const auto itPrev = std::prev(scroll);

                            if ((*itPrev == s[0U]) && (s[1U] == next)) {
                                count = 2U;
                            }
                        }
                    }
    ...
    unsigned lineCountFirstChars{0U};
    ...
    while ((*inputStream) >> line) { // suppose no space in each line
        ...
        const auto [firstDigit, posFirst]
            = charsOrDigit(line.begin(), line.end(), Direction::Front);

        if (posFirst == line.end()) {
            throw std::invalid_argument(errorLine + "no digit!");
        }

        const auto [lastDigit, posLast]
            = charsOrDigit(line.rbegin(), line.rend(), Direction::Back);
        ...
        auto v = firstDigit * BaseDec + lastDigit;
        sum += v;

        if (v == 0U) {
            ++lineCountZero;
        } else if (firstDigit == 0U) {
            ++lineCountLeadingZero;
        }

        if (!std::isdigit(*posFirst)) {
            ++lineCountFirstChars;
        }
        if (!std::isdigit(*posLast)) {
            ++lineCountLastChars;
        }
        if (!(std::isdigit(*posFirst) || std::isdigit(*posLast))) {
            ++lineCountBothChars;
        }
    }
...
example:
Number of lines: 7
N. of lines with one digit only: 0
N. of lines with value zero: 0
N. of lines with leading zero: 0
N. of lines with first chars: 5
N. of lines with last chars: 5
N. of lines with both chars: 4
Result: 281

Real input:
Number of lines: 1000
N. of lines with one digit only: 54
N. of lines with value zero: 0
N. of lines with leading zero: 0
N. of lines with first chars: 451
N. of lines with last chars: 440
N. of lines with both chars: 183
Result: 54875
*/
