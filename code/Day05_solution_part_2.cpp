...
auto day05Part2(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    std::map<StartDest, RangeLen> currentDest;
    Mapping translation;

    const auto translate = [&currentDest, &translation](const std::string& tokenTitle) {
        // translate the old set
        std::map<StartDest, RangeLen> newDest;
        if (!translation.empty()) {
            auto it = translation.cbegin();

            for (auto [startRefC, len] : currentDest) {
                auto start = startRefC; // as referred keys are const StartDest &.

                if (it != translation.end()) {
                    if (it->first < start) {
                        // new it [TODO: optimize with initial pos 'it']
                        it = translation.lower_bound(start + 1U);
                        --it;
                        {
                            const auto source = it->first;
                            const auto range = it->second.second;
                            if (source + range <= start) {
                                // The overall previous range does not overlap
                                // and this would wrongly break soon in (_1_)
                                // without such a ++.
                                ++it; // This is the result of lower_bound
                                        // and may not be overlapped as well.
                            }
                        }
                    }
                    // it now is the first that overlap or that is righter to the current range.

                    /*if (it != translation.end()){
                        std::cout << "first range for (" << start << "," << len << ") has"
                                    << " source: " << it->first << " and dest: " <<
                    it->second.first << std::endl;
                    }*/

                    while ((len > 0) && (it != translation.end())) {
                        const auto source = it->first;
                        const auto dest = it->second.first;
                        const auto range = it->second.second;

                        // the current range may be broken in multiple ranges.

                        // Second clause useful only first time.
                        if ((source < start + len) && (source + range > start)) {
                            // to be broken
                            ...
                            //             newEnd-newStart << ") "
                            //             << "using source: " << source << " and dest: " <<
                            //             dest << std::endl;

                            // residual right part to be checked with next ranges
                            len -= (newEnd - start);
                            start = newEnd;
                        } else {
                            break; // (_1_)
                        ...
                        if (len > 0) {
                            ...
                            // The same 'it' may be used for next currente range.
                            break;
                        }
                    }
                }
        ...
        std::cout << "New translated number of ranges " << newDest.size() << " before title "
                    << tokenTitle << std::endl;
    ...
    while (inputStream->getline(cc.data(), MaxLineLength, '\n')) {
        ...
        if (std::isdigit(tokenTitle[0])) {
            ...
            std::string tokenNumber2;
            while (lineStream >> tokenNumber) {
                Value seedStart{};
                if (!toUnsigned(tokenNumber, seedStart)) {
                    throw std::invalid_argument(
                        // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                        errorLine + "not a seed number: " + tokenNumber);
                }

                if (!(lineStream >> tokenNumber2)) {
                    throw std::invalid_argument(errorLine + "absent seed-range");
                }
                Value seedRange{};
                if (!toUnsigned(tokenNumber2, seedRange)) {
                    throw std::invalid_argument(
                        // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                        errorLine + "not a seed-range number: " + tokenNumber2);
                }
                ...
                if (it != currentDest.begin()) {
                    ...
                    if (it->first + it->second > seedStart) {
                        throw std::invalid_argument(
                            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                            errorLine + "overlapped range before (" + tokenNumber + ','
                            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                            + tokenNumber2 + ')');
                    }
                    ++it; // back to the new seed range.
                }
                ++it;
                if (it != currentDest.end()) {
                    if (seedStart + seedRange > it->first) {
                        throw std::invalid_argument(
                            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                            errorLine + "overlapped range after (" + tokenNumber
                            + ','
                            // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                            + tokenNumber2 + ')');
                    }
            ...
            std::cout << "Initial number of seeds-range " << currentDest.size() << std::endl;
            nSeedRanges = currentDest.size();
        } else {
    ...
    std::cout << "N. of initial seed ranges " << nSeedRanges << std::endl;
...
int main05p2()
{
    try {
        ...
        // day05Part2("./05_input_file.txt",true);
...
Example:
Number of lines: 33
N. of initial seed ranges 2
N. of total final positions: 7
...
Real input:
Number of lines: 190
N. of initial seed ranges 10
N. of total final positions: 91
N. of repeated seeds: 0
