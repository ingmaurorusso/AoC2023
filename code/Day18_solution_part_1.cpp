...
auto day18Part1(std::string_view streamSource, bool sourceIsFilePath)
{
    ...
    std::vector<std::string> linesDir;
    Coord fieldLength{};
    ...
    currPos = startP;
    {
        const auto dotLine = std::string(startX + 1U, '.');
        for (Coord y = 0U; y <= startY; ++y) {
            linesDir.push_back(dotLine);
            lines.push_back(dotLine);
        }
        lines[startY].back() = '#'; // first cube
    }
    ...
    for (const auto& digMove : digMoves) {
        ...
        linesDir[currPos.y][currPos.x] = getTurn(oldDir, dir, lineCount==1U );
        ...
        {
            auto add = std::string(lines[0].size(), '.');
            while (newP.y + 1U > lines.size()) {
                lines.push_back(add);
                linesDir.push_back(add); // last might move
            }
        }

        if (newP.x + 1U > fieldLength) {
            auto add = std::string(newP.x + 1U - fieldLength, '.');
            // cppcheck-suppress useStlAlgorithm
            for (auto& l : lines) {
                l += add; // might directly be '#'
            }
            fieldLength = newP.x + 1U;

            // cppcheck-suppress useStlAlgorithm
            for (auto& l : linesDir) {
                l += add; // might move last time.
            }
        }

        bool cross = false;

        const auto checkCross
            = [startP, newP, &lines, lastMove = (lineCount == digMoves.size())](const Point p) {
                    return (lines[p.y][p.x] == '#')
                        && ((p != newP) || (newP != startP) || !lastMove);
                };

        switch (dir) {
        case Dir::Right:
            while (newP.x > currPos.x) {
                ++cubes;

                ++currPos.x;
                cross |= checkCross(currPos);

                lines[currPos.y][currPos.x] = '#';
                linesDir[currPos.y][currPos.x] = '-';
            }
            break;
        case Dir::Down:
        ...
        if (cross) {
            // TODO: in case of cross, ignore linesDir and apply a phase
            // where the envelop of 'lines' is searched, so regenerating linesDir
            // before applying the final search.
            throw std::runtime_error(
                "Cannot manage yet crosses: implement pre-envelope search");
        }
    ...
    if (currPos != startP) {
        ...
        std::cout << "last point " << pointToStr(currPos) << std::endl;
    ...
    const auto nCols = fieldLength;
    for (Coord y = 0U; y < nRows; ++y) {
        const auto& lineDir = linesDir[y];
        auto& line = lines[y];
        bool inUp = false;
        bool inDown = false;
...
int main18p1()
{
    try {
        ...
        // day18Part1("./18_input_file.txt",true);
